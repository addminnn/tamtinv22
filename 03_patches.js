    ...
    if(btnSend && !btnSend.__helpPatched){
      ...
      btnSend.addEventListener("click", ()=>{
        ...
        if(res.ok){
          ...
          // generate package for cross-device transfer (optional)
          try{
            const arr = loadHelpTickets();
            const t = arr.find(x=>x && x.id===res.ticketId) || arr[arr.length-1];
            const pkg = encodeHelpPackageFromTicket(t||{});
            const wrap = $("helpPkgWrap");
            const ta = $("helpPackage");
            if(wrap && ta){
              ta.value = pkg || "";
              wrap.style.display = pkg ? "block" : "none";
            }
            const st = $("helpPkgStatus");
            if(st){
              st.textContent = pkg
                ? "Gợi ý: nếu GV dùng máy khác, bạn sao chép mã này gửi cho GV để nhập."
                : "—";
            }
            // best effort auto-copy
            if(pkg && navigator.clipboard && navigator.clipboard.writeText){
              navigator.clipboard.writeText(pkg).then(()=>{
                try{ if(st) st.textContent = "✅ Đã sao chép mã vào clipboard (có thể dán gửi GV)."; }catch{}
              }).catch(()=>{});
            }
          }catch{}
        } else {
